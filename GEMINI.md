...
1.  **コマンド実行**: プレイヤーがコマンド (`.set stone` など) を実行すると、`eventListener.ts` がイベントを捕捉し、`customCommands.ts` に登録された対応するコマンド実装 (`impl/` 以下) を呼び出します。
2.  **ツール使用**: プレイヤーがツール（例: 木の斧）を使用すると、イベントハンドラ (`handlers/`) が `PlayerInfoManager` を参照してツールを特定し、対応するロジック (`tools/` 以下) を実行します。
3.  **履歴管理**: ブロック操作の直前に `HistoryManager` が変更前の状態を `BlockDataManager` を介して保存します。`.undo` や `.redo` が実行されると、保存された状態を復元します。
